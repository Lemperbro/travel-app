      ...
      <div class=" grid grid-cols-1 sm:grid-cols-2  lg:grid-cols-3 xl:grid-cols-4 gap-5">
        ...
        {{-- card 2 start --}}
        @foreach ($data as $wisatas)
        @php
        $images = explode('|', $wisatas->image);
        $event = App\Models\Event::where('wisata_id', $wisatas->id)->where('tipe', 'aktif')->where('status', 1)->first();

        $session = App\Models\Session::where('wisata_id',$wisatas->id)->where('startDate', '<=',Carbon\Carbon::now()->format('Y-m-d'))->where('endDate', '>=',Carbon\Carbon::now()->format('Y-m-d'))->where('type', 'session')->first();
                    
        $weekend = App\Models\Session::where('wisata_id',$wisatas->id)->where('type', 'weekend')->first();
          ...
          <div class="mt-2">
            <dl>
              ...
              <div>
                ...
                <dd class="text-gray-900 dark:text-white font-semibold text-xl">{{ $wisatas->nama_wisata }}</dd>
              </div>
              <div>
            ...
            <div class="mt-6 grid grid-cols-3 items-center gap-8 text-xs">
              <div class="sm:inline-flex sm:shrink-0 sm:items-center sm:gap-2">
                ...
                <div class="mt-1.5 sm:mt-0">
                  ...
                  <p class="font-medium text-gray-700 dark:text-gray-300">{{ $wisatas->kota->nama_kota }}</p>
                </div>
              </div>
              ...
              <div class="sm:inline-flex sm:shrink-0 sm:items-center sm:gap-2">
                ...
                <div class="mt-1.5 sm:mt-0">
                  ...
                  <p class="font-medium text-gray-700 dark:text-gray-300">{{ $wisatas->long_tour }}</p>
                </div>
              </div>
              ...
              <div class="sm:inline-flex sm:shrink-0 sm:items-center sm:gap-2">
                ...
                <div class="mt-1.5 sm:mt-0">
                  ...
                  <p class="font-medium text-gray-700 dark:text-gray-300">{{ $wisatas->tour_type }}</p>
                </div>
              </div>
            </div>
          </div>
