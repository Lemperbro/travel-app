<div class=" grid grid-cols-1 sm:grid-cols-2  lg:grid-cols-3 xl:grid-cols-4 gap-5">
    ...
    {{-- card 2 start --}}
    @foreach ($best->take(5) as $best_wisata)
        @php
            $images = explode('|', $best_wisata->image);
            $event = App\Models\Event::where('wisata_id', $best_wisata->id)
                ->where('tipe', 'aktif')
                ->where('status', 1)
                ->first();
            
            $session = App\Models\Session::where('wisata_id', $best_wisata->id)
                ->where('startDate', '<=', Carbon\Carbon::now()->format('Y-m-d'))
                ->where('endDate', '>=', Carbon\Carbon::now()->format('Y-m-d'))
                ->where('type', 'session')
                ->first();
            
            $weekend = App\Models\Session::where('wisata_id', $best_wisata->id)
                ->where('type', 'weekend')
                ->first();
        ...
        <a href="/wisata/{{ $best_wisata->slug }}" class="block rounded-lg p-4 shadow-best dark:bg-gray-700 bg-white">
            <img alt="Home" src="{{ asset('image/' . $images[0]) }}" class="h-56 w-full rounded-md object-cover" />
            ...
            <div class="mt-2">
                <dl>
                    ...
                    <div>
                        ...
                        <dd class="text-gray-900 dark:text-white font-semibold text-xl">{{ $best_wisata->nama_wisata }}
                        </dd>
                    </div>
                    <div>
                ...
                <div class="mt-6 grid grid-cols-3 items-center gap-2 text-xs">
                    <div class="sm:inline-flex sm:shrink-0 sm:items-center sm:gap-2">
                        ...
                        <div class="mt-1.5 sm:mt-0">
                            ...
                            <p class="font-medium text-gray-700 dark:text-gray-300">{{ $best_wisata->kota->nama_kota }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="sm:inline-flex sm:shrink-0 sm:items-center sm:gap-2">
                        <div class="relative inline-block">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                class="fill-orange-600 w-7 h-7 " style="transform: ;msFilter:;">
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                class="absolute top-3 -right-1 w-4"
