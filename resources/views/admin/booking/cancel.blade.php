  ...
  <div class="mx-auto p-2">
    ...
    <div class="flex flex-wrap flex-row">
      <div class="flex-shrink max-w-full px-4 w-full">   
        <p class="text-xl font-bold mt-3 mb-5 dark:text-white">Cancel</p>
      </div>                                                 
                    ...
                    <tbody>
                      ...
                      <tr class="border-b-[1px] border-gray-300 dark:border-gray-700">
                        ...
                        <td class="hidden lg:table-cell text-center">
                          <span class="text-sm px-2 py-1 font-semibold leading-tight {{ ($client->status == 'menunggu') ? 'text-yellow-700 bg-yellow-100' : '' }} {{ ($client->status == 'ditolak') ? 'bg-red-600 text-white' : '' }} {{ ($client->status == 'cancel') ? 'bg-red-600 text-white' : '' }} rounded-full">{{ $client->status }}</span>
                        </td>
                        ...
                        <td class="text-center ">
                          ...
                          <form method="Post" action="/admin/booking/cancel/{{ $client->id }}" class="hidden">
                            @csrf
                            <button type="submit" class="hidden">
                              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" class="hidden" viewBox="0 0 24 24" style="fill: rgba( 0, 128, 0, 1);transform: ;msFilter:;"><path d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm-1.999 14.413-3.713-3.705L7.7 11.292l2.299 2.295 5.294-5.294 1.414 1.414-6.706 6.706z"></path></svg>
                            </button>
                          </form>

                          <form action="/admin/booking/cancel/{{ $client->id }}" method="POST">
                            @csrf
                            <button type="submit">
